...
import { perfProbe, SLOW_FRAME_MS, type PerfSnapshot } from './perfProbe';
...
export function PerfMeter() {
  ...
  useFrameCallback((frame) => {
    'worklet';
    if (uiReset.value) {
      uiMax.value = 0; uiSlow.value = 0; uiFrames.value = 0; uiReset.value = 0;
    }
  ...
  useEffect(() => {
    ...
    const refresh = setInterval(() => {
      perfProbe.recordUiFrames(uiMax.value, uiSlow.value, uiFrames.value);
      setSnap(perfProbe.snapshot());
    }, REFRESH_MS);
    return () => { clearInterval(tick); clearInterval(refresh); };
  }, [uiMax, uiSlow, uiFrames]);
